...
# Database setup
def get_db():
    db_path = os.environ.get('DATABASE_URL', 'instance/knitwear_erp.db')
...
def login():
    ...
    with get_db() as db:
        ...
        if user and check_password_hash(user['password_hash'], password):
            ...
            db.execute('UPDATE users SET last_activity = ? WHERE id = ?', (datetime.now(), user['id']))
            db.commit()
...
def add_musteri():
    ...
    if not customer_no:
        customer_no = f"CUST{datetime.now().strftime('%Y%m%d%H%M%S')}"
    
    with get_db() as db:
        # Check if customer number already exists
        existing = db.execute('SELECT id FROM customers WHERE customer_no = ?', (customer_no,)).fetchone()
        if existing:
            return jsonify({'success': False, 'message': 'Müşteri numarası zaten mevcut'}), 400
        
        # Add customer
        cursor = db.execute('''
            INSERT INTO customers (customer_no, name, phone, address, email)
            VALUES (?, ?, ?, ?, ?)
        ''', (customer_no, data.get('name'), data.get('phone'), data.get('address'), data.get('email')))
        db.commit()
        ...
        return jsonify({
            ...
            'musteri': {
                'id': cursor.lastrowid,
                'musteri_no': customer_no,
                'musteri_adi': data.get('name')
            }
        })
...
def approve_model(model_id):
    ...
    with get_db() as db:
        ...
        db.execute('''
            ...
            WHERE id = ?
        ''', (session['user_id'], datetime.now(), model_id))
